...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = async (book, quantity = 1) => {
    if (user) {
      try {
        const response = await api.post(`/api/cart/${user.id}/add`, {
          bookId: book._id,
          quantity
        });
        setCart(response.data);
  ...
  const updateQuantity = async (bookId, quantity) => {
    if (user) {
      try {
        const response = await api.put(`/api/cart/${user.id}/update`, {
          bookId,
          quantity
        });
        setCart(response.data);
  ...
  const removeFromCart = async (bookId) => {
    if (user) {
      try {
        const response = await api.delete(`/api/cart/${user.id}/remove/${bookId}`);
        setCart(response.data);
      } catch (error) {
